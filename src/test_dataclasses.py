...
print(f"INFO: {datetime_now}: Starting import of Dataclasses")
...
for response_file_name in files_to_test:
    ...
    print(
        f"INFO: {datetime_now}: Starting pydantic-to-Dataclass validation of local JSON file {response_file_name} of size {os.path.getsize(response_file_name) / 1024 / 1024} MB"
    )
    adapter = pydantic.TypeAdapter(Response)
    t0 = timeit.default_timer()
    if response_file_name.endswith(".gz"):
        with gzip.open(response_file_name, "rt", encoding="utf-8") as infile:
            response_dict = adapter.validate_json(infile.read())
    else:
        with open(response_file_name) as infile:
            response_dict = adapter.validate_json(infile.read())
    t1 = timeit.default_timer()
