...
@pytest.mark.parametrize(
    "model", _UPDATE_MODELS, ids=[m.__name__ for m in _UPDATE_MODELS]
)
def test_update_with_empty_is_identity(model: type[TOMBase]) -> None:
    ...
    x = build(model)
    before = x.hash()
    x.update(empty)  # defaults; ignore any (old:new) mapping tuple some updates return
...
@pytest.mark.parametrize(
    ("model", "dictutil"),
    _UPDATE_DICTUTILS,
    ids=[du.__name__ for _, du in _UPDATE_DICTUTILS],
)
...
@pytest.mark.parametrize(
    "model", _NORMALIZE_MODELS, ids=[m.__name__ for m in _NORMALIZE_MODELS]
)
...
@pytest.mark.parametrize(
    ...
    ids=[du.__name__ for _, du in _NORMALIZE_DICTUTILS],
)
...
@pytest.mark.parametrize(
    "model", _MERGE_MODELS, ids=[m.__name__ for m in _MERGE_MODELS]
)
...
@pytest.mark.parametrize(
    ("model", "dictutil"),
    _MERGE_DICTUTILS,
    ids=[du.__name__ for _, du in _MERGE_DICTUTILS],
)
