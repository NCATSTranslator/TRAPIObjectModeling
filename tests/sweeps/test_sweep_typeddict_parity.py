...
def _typeddict_for(model: type[TOMBase]) -> Any:
    """The `{Name}Dict` TypedDict mirroring `model` (in its own version), or None."""
    version = model.__module__.split(".")[1]
    model_dicts = importlib.import_module(f"translator_tom.{version}.model_dicts")
    return getattr(model_dicts, f"{model.__name__}Dict", None)
...
@pytest.mark.parametrize("model", PAIRED, ids=[sweep_id(m) for m in PAIRED])
