...
import pytest
from _sweep_helpers import sweep_id, DICTUTILS, MODELS

import translator_tom.v1_6
...
EXPECTED_NO_DICTUTIL: set[type] = {
    # 1.6 abstract base for QueryGraph/PathfinderQueryGraph; never serialized on its own
    # (a query graph always carries `edges` or `paths`). Both concrete subclasses have
    # DictUtils, so the base intentionally has none. (2.0 collapsed these into a single
    # concrete QueryGraph, so 2.0 has no such exemption.)
    translator_tom.v1_6.BaseQueryGraph,
}
