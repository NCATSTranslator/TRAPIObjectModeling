...
import pytest
from _sweep_helpers import DICTUTILS, MODELS, build, version_of
...
_ACCESSOR_IDS = [f"{version_of(m)}:{m.__name__}.{n}" for m, n in _ACCESSORS]
...
def _parity_param(model: type[TOMBase], accessor: str) -> pytest.ParameterSet:
    ...
    return pytest.param(
        model, accessor, marks=marks, id=f"{version_of(model)}:{model.__name__}.{accessor}"
    )
