...
@pytest.mark.parametrize("model", CONSTRUCT_PARAMS, ids=[sweep_id(m) for m in MODELS])
...
@pytest.mark.parametrize("model", NEW_PARAMS, ids=[sweep_id(m) for m in NEW_MODELS])
